...
int main(int argc, char *argv[])
{
    ...
    /* Creates data to be used and associated with the client */
    ...
    while (opt < argc)
    {
        ...
        case 'b':
            ...
            if (!serverPortChanged) serverPort = LWM2M_BSSERVER_PORT_STR; //Sends a Bootstrap request to the server as long as Port Number has remained the same
            break;
        case 'c':
    ...
    // sleep(10);
    ApiInit();
    int senpwr_handle = sen_power_open("sensor_Vs");	// From Friendly Name from Morpheus App Builder
    int ai1_handle = analog_open("pressure_reading");	// From Friendly Name from Morpheus App Builder
    ...
    /*  Obtains connection credentials to establish with LWM2M Server */  
    ...
    maxSensor = analog_process_config.AI1.max_sensor;
    ...
    /* The liblwm2m library is now initialized with the functions that will be in charge of communication */
    ...
    while (0 == g_quit)
    {
        ...
        else if (batterylevelchanging)
        {
            ...
            tv.tv_sec = 5;  //speed entry back to main loop
        }
        else
        {
            tv.tv_sec = 5;  //speed entry back to main loop
		...
		/* This function takes in the analog input as a voltage and converts it to PSI for display */
        ai1_counts = read_analog_counts(ai1_handle);
        ai1_millivolts = read_analog_millivolts(ai1_handle);
        /* update resource value within the object 3323 */
        targetP = (prv_instance_t *)lwm2m_list_find(objArray[5]->instanceList, 0);
        if (ai1_millivolts >= 4500)    //defines the max voltage to convert to 1000 PSI
        {
            targetP->sensorValue = 1000.0;
        }
        else if (ai1_millivolts <= 500)    //defines the min voltage to convert to 0 PSI
        {
            targetP->sensorValue = 0;
        }
        else
        {
            targetP->sensorValue = ((ai1_millivolts - 500) / 4000) * 1000;    //Calculates the actual PSI from sensor    
        }
        /* This part is needed to trigger the watcher of the observable */
        if (lwm2m_stringToUri("/3323/0/5700", 12, &uri))
        {
            valueLength = sprintf(value, "%f", targetP->sensorValue);
            fprintf(stderr, "New Pressure: %f\n", targetP->sensorValue);
            lwm2m_resource_value_changed(lwm2mH, &uri);
		...
		/* The switch below returns the state of the LWM2M Bootstrap */
        switch (lwm2mH->state)
        {
        case STATE_INITIAL:
